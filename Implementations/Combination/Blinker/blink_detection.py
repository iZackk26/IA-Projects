...
class EyeDetector:
    ...
    def __init__(self, raw_faces, frame, gray, counter, total):
        ...
        if not hasattr(self, "initialized"):
            ...
            self.last_detected = ""
            self.total = total
            self.predict_eyes = dlib.shape_predictor(
    ...
    def detect_blink(self):
        ...
        if processed_faces:
            boxes_faces = self.convert_rectangles2array(
                self.process_images(), self.frame
            )
            areas = self.get_areas(boxes_faces)
            boxes_faces = np.expand_dims(boxes_faces[0], axis=0)
            self.eye_blinker(
                self.process_images()[0], self.gray, self.counter, self.total
            )
            return self.frame, boxes_faces, [self.last_detected]
